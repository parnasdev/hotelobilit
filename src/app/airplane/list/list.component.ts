...
export class ListComponent {
  ...
  ngOnInit(): void {
    this.title.setTitle('هواپیما ها | هتل و بلیط')
  ...
  getTransfers(): void {
    this.setRea();
    this.isLoading = true
    this.api.getCategoryList('airplane', 'hotel', this.p).subscribe((res: any) => {
      if (res.isDone) {
  ...
  deleteAirplane(id: number) {
    this.api.deleteCategory(id, 'airplane').subscribe((res: any) => {
      if (res.isDone) {
        this.message.custom(res.message);
        this.getTransfers()
      } else {
